...
extern "C"
JNIEXPORT void JNICALL
Java_grad_1project_myapplication_OcrActivity_bounding(JNIEnv *env, jobject thiz, jlong input_mat) {
    ...
    __android_log_print(ANDROID_LOG_DEBUG, "native-lib :: ",
                        "%d selected_points_size", selected_points.size());

    for(int i = 0; i< selected_points.size(); i++) {
        __android_log_print(ANDROID_LOG_DEBUG, "native-lib :: ",
                            "%d selected_points - x", selected_points[i].x);
        __android_log_print(ANDROID_LOG_DEBUG, "native-lib :: ",
                            "%d selected_points - y", selected_points[i].y);
        line(img, selected_points[i], selected_points[(i + 1) % 4], Scalar(0, 0, 255), 2);
    }
}